...
class ListUsersApplicationsAdapter(private var mApplications: List<ApplicationsDAO>?, con: BaseActivity,
    ...
    override fun onBindViewHolder(holder_parent: ParentViewHolder, position: Int) {
        ...
        if (searched_text != null && searched_text!!.length > 0) {
            ...
            holder.category.text = Shared.getInstance().getSearchedTextHighlight(searched_text, category)
            holder.definitionName.text = Shared.getInstance().getSearchedTextHighlight(searched_text, definitionName)
            holder.applicationNumber.text = Shared.getInstance().getSearchedTextHighlight(searched_text, applicationNumber)
